...
public interface IServerAPICalls
{
    ...
    string FindDSCResourceNameWithNoPrerelease(string[] dscResourceName, PSRepositoryInfo repository, out string[] errRecord);
    ...
    /// Examples: Search -Name "DSCResource1", "DSCResource2" -Repository PSGallery
    /// - Include prerelease: http://www.powershellgallery.com/api/v2/Search()?$filter=IsLatestVersion&searchTerm='Az* tag:PSDSCResource_DSCResource1 tag:PSDSCResource_DSCResource2'&includePrerelease=true
    string FindDSCResourceNameWithPrerelease(string[] dscResourceName, PSRepositoryInfo repository, out string[] errRecord);
