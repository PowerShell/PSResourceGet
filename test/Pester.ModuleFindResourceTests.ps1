...
Describe 'Test Find-PSResource for Module' {
    ...
    It "find resource when given ModuleName, Version not null --> <Version>, and name is of a script resource" -TestCases @(
        @{Version="[2.0.0.0]"; ModuleName="Fabrikam-ServerScript"}
        # @{Version="[2.0.0.0]"; ModuleName = "ContosoServer"}
    ) {
        param($Version, $ModuleName)

        $res = Find-PSResource -ModuleName $ModuleName -Repository (Get-PoshTestGalleryName) -Version $Version
        $res | Should -BeNullOrEmpty
    }
