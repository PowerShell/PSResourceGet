...
Describe 'Test Find-PSResource for Role Capability' {
    ...
    It "not find resource and throw exception with incorrectly formatted version such as <Description>" -TestCases @(
        ...
        param($Version, $Description)
        { Find-PSResource -Name "test_rolecap_module" -Version $Version -Repository $TestGalleryName -ErrorAction Ignore } | Should -Throw -ExceptionType ([ArgumentException])
    }
    ...
    It "find resources when given Name, Version not null --> '*'" {
        ...
        $actualModules.Count | Should -Be $RoleCapTest.Count
    }
